...
#include "pyphy/likelihood/mcmc_updater.hpp"		// for MCMCUpdaterShPtr (plus LotShPtr, ModelShPtr, TreeShPtr, TreeLikeShPtr, etc.)
#include "pyphy/likelihood/mcmc_param.hpp"			// for MCMCUpdaterShPtr (plus LotShPtr, ModelShPtr, TreeShPtr, TreeLikeShPtr, etc.)
...
#		endif
	...
	private:
		...
		MCMCUpdaterVect			all_updaters;			/**< Vector of all updaters (includes moves, model_params, edge_len_params and, if used, edge_len_hyperparam); empty until finalize() is called, after which it contains a copy of everything in moves, model_params, edge_len_params and edge_len_hyperparam */
	};
...
#include "pyphy/likelihood/mcmc_chain_manager.inl"
