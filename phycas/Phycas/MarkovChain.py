		...
		if (nmodels > 1):
			...
			if partition.fix_subset_relrates:
				...
				if partition.subset_relrates_prior is None:
					...
					#d = Dirichlet(param_list)	#OLDWAY
					d = ProbDist.RelativeRateDistribution(param_list)	#NEWWAY
					self.subset_relrates_move.setMultivarPrior(d)
					self.partition_model.setSubsetRelRatePrior(d)
				else:
					# self.parent.phycassert(partition.subset_relrates_prior.getDistName() == 'Dirichlet', 'partition.subset_relrates_prior must be of type Dirichlet')	#OLDWAY
					self.parent.phycassert(partition.subset_relrates_prior.getDistName() == 'RelativeRateDistribution', 'partition.subset_relrates_prior must be of type RelativeRateDistribution')	#NEWWAY
