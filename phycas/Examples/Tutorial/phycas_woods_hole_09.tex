...
\fancyhead[RO,LE]{2009 Workshop on Molecular Evolution Woods Hole}
\fancyfoot[LO,LE]{{\footnotesize \copyright\ 2008-2009 by Paul O. Lewis, Mark T. Holder, and David Swofford}}
\fancyfoot[CE,CO]{}
...
To unpack the archive and set up your environment use the following commands:\par
{\tt
...
All the Phycas.app application is doing for you is: launching a terminal window, feeding the terminal the \cmd{python} command, and finally telling python ``\cmd{from phycas import *}''.
...
Note that you {\bf still must} use this import command in all of the \phycas scripts that you write.
...
\end{itemize}
The lab machines are configured to use IPython.  If you install \phycas and IPython on your own machine, then you will need to indicate to \phycas that you would like to use IPython.
To do this,  open the \localfile{$\sim$/.phycas/active\_phycas\_env.sh} file
...
As in \mb, we can summarize the trees produced by the \cmd{mcmc} analysis
...
\localfile{Polytomy.py}, \localfile{basic.py}, and  \localfile{steppingStone.py}
...
Currently \phycas only support a uniform prior over all tree topologies (within a
...
We could use an Exponential (or Gamma distribution) for the prior on all of the branch lengths, but we will also have to choose a mean value for the prior distribution.
Choosing a reasonable prior is important for every  parameter in a Bayesian analysis, but 
...
We can control the behavior of the summarization with:
\begin{verbatim}
...
With this prior, you specify the ratio of the prior probability of a tree over the probability of another tree that is identical to it except  that it has one fewer internal branch.
...
Make sure to rename your output files, so that we do not overwrite the files that we just produced
...
The easiest way to do this on Mac with the Phycas.app bundle is to simply drag the \localfile{Polytomy.py} file onto the \phycas icon (in the dock if \phycas is running or on the to the application icon if it is not running).
This will set the working directory for the \phycas run to be the directory that contains the script that was dragged onto the icon (so you may have to use absolute paths to data files in the script, if the data files are not in the same directory). 
...
In fact, they suggest that the prior be used to prefer shorter internal branches when you have a large number of characters in your data set.
...
For some datasets, the priors suggested by Yang and Rannala will result in a majority-rule consensus tree that is less resolved than the Bayesian analysis with polytomies forbidden and  a vague prior over branch lengths.
...
At this point you should open the \localfile{scripts/steppingStone.py} file in a text editor.
Read the file and comments (any characters after a \# are considered comments in Python).
