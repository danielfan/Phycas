...
mcmc.data_source = 'rokas' + ntax + '.nex'
mcmc.out.log = model.type + ntax + 'output.%d.txt' % (jobid,)
mcmc.out.log.mode = REPLACE
mcmc.out.trees.prefix = model.type + ntax + 'trees.%d' % (jobid,)
mcmc.out.trees.mode = REPLACE
mcmc.out.params.prefix = model.type + ntax +  'params.%d' % (jobid,)
...
if not regular_mcmc and jobid <= ntrees:
...
if ntrees == 105:
    ...
    tree_descriptions = [
        '(1:0.063233,(4:0.050563,3:0.111184):0.03100,2:0.144445)',
        '(1:0.063233,(2:0.050563,4:0.111184):0.03100,3:0.144445)',
...
if regular_mcmc:
    mcmc()
    '''sump.out.log = model.type + ntax + 'mcmc.sump.%d.txt' % (jobid,)
    sump.out.log.mode = REPLACE
    sump.file = model.type + ntax + 'mcmc.params.%d.p' % (jobid,)
    sump()'''
    sumt.burnin = 5
    sumt.out.refdistfile.prefix = "refdist"
    sumt.out.refdistfile.mode = REPLACE
    sumt.trees = model.type + ntax + 'trees.' + str(jobid) + '.t'
    sumt.out.log = model.type + ntax + 'mcmc.sumt.%d.txt' % (jobid,)
    ...
    if testing_sample:
        ...
        if larget_simon:
            sumt.out.log = model.type + ntax + 'trees_from_refdist_larget_simon.%d.txt' % (jobid,)
        else:
            sumt.out.log = model.type + ntax + 'trees_from_refdist_direct_draw.%d.txt' % (jobid,)
        sumt.out.log.mode = REPLACE
        sumt.tree_credible_prob = 1.0
        sumt()
    else:
        sump.out.log = model.type + ntax + 'ss.sump.%d.txt' % (jobid,)
        sump.out.log.mode = REPLACE
        sump.file = model.type + ntax +  'params.%d.p' % (jobid,)
        sump()
