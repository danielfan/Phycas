 ...
 **************************/
    ...
    public void setGridColor(Color d) {
        gridcolor = d;
    }
    /**
    ...
    public void setZeroColor(Color d) {
        zerocolor = d;
    }
    /**
    ...
    public void setDataWindow(Dimension w) {
        data_window = w;
    }
 ...
 ******************/
     ...
     */
    public int getAxisWidth(Graphics2D g) {
        int i;
        width = 0;
    ...
    /**
     * Draw the axis using the passed Graphics2D context.
 ...
 ********************/
    ...
    protected void drawHAxis(Graphics2D g) {
        //Graphics2D lg;
        int i;
        int j;
        int x0,y0,x1,y1;
        ...
        g.draw(new Line2D.Double(amin.x,amin.y,amax.x,amax.y));
        ...
        for(i=0; i<label_count; i++) {
            if( val >= vmin && val <= vmax ) {
                ...
                if( Math.abs(label_value[i]) <= 0.0001 && drawzero ) {
                    ...
                    g.draw(new Line2D.Double(x0,y0,x0,y0+data_window.height*direction));
    ...
    protected void drawVAxis(Graphics2D g) {
        //Graphics2D lg;
        int i;
        int j;
        int x0,y0,x1,y1;
        ...
        for(i=0; i<label_count; i++) {
            if( val >= vmin && val <= vmax ) {
                ...
                if( Math.abs(label_value[i]) <= 0.0001 && drawzero ) {
                    c = g.getColor();
                    if(zerocolor != null) g.setColor(zerocolor);
                    g.draw(new Line2D.Double(x0,y0,x0+data_window.width*direction,y0));
    ...
    protected void calculateGridLabels() {
        ...
        if (Math.abs(minimum) > Math.abs(maximum) )
            label_exponent = ((int)Math.floor(SpecialFunction.log10(Math.abs(minimum))/3.0) )*3;
        else
            label_exponent = ((int)Math.floor(SpecialFunction.log10(Math.abs(maximum))/3.0) )*3;
