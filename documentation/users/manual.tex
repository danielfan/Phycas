...
\newcommand{\currReleaseDate}{15 July 2010}		% TODO: CHECK PRIOR TO NEW RELEASE
...
% TODO: remove this section as soon as possible
\begin{samepage}
\begin{verbatim}
model.edgelen_hyperprior = None
model.edgelen_prior = Exponential(1.0)
\end{verbatim}
\pointup\ These two lines are necessary because Phycas does not currently allow the use of edge length hyperpriors (see section~\ref{hierarchicalmodels}) when the tree topology is fixed. This restriction will be lifted in the next release, but for now we must choose a prior (here, an exponential distribution with rate 1.0) for the edge length parameters and turn off the hyperprior by specifying \code{None} for \opt{model}{edgelen\_hyperprior}. 
\end{samepage}
...
mcmc.ncycles = 1000
\end{verbatim}
\pointup\ The \cmd{ss} command uses the \cmd{mcmc} to do almost all of the work. Hence, the setting \opt{mcmc}{ncycles} specifies the number of parameter update cycles devoted to each beta value visited. Note that this is the number of cycles per beta value. If you specified \opt{ss}{nbetavals} to be 11 and \opt{mcmc}{ncycles} to be 1000, then the total number of cycles would be 11 times 1000, or 11000 cycles.
\end{samepage}
