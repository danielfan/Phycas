...
#analysis_prior_means = [0.00001, 0.00005, 0.0001]
...
class analyze(threading.Thread):
    ...
    def run(self):
        ...
        analyzer.master_edgelen_dist = ProbDist.ExponentialDist(1.0/self.mean)
        ...
        self.Pm = analyzer.gg_Pm
        self.Gm = analyzer.gg_Gm[0]
...
if __name__ == '__main__':
    ...
    nmeans = len(analysis_prior_means)
    for i in range(0, nmeans, 2):
        even_prior_mean = analysis_prior_means[i]
        even_thread = analyze(even_prior_mean)
        even_thread.start()
        if i+1 < nmeans:
            odd_prior_mean = analysis_prior_means[i+1]
            odd_thread = analyze(odd_prior_mean)
            odd_thread.start()
            odd_thread.join()
            oddPm = odd_thread.Pm
            oddGm = odd_thread.Gm
            oddGGCm = oddPm + oddGm
            outf.write('%f\t%f\t%f\t%f\n' % (odd_prior_mean, oddPm, oddGm, oddGGCm))
            print '**'
            print '** Analysis with prior mean',odd_prior_mean,'yielded Pm =',oddPm,', Gm =',oddGm
            print '**'
        even_thread.join()
        evenPm = even_thread.Pm
        evenGm = even_thread.Gm
        evenGGCm = evenPm + evenGm
        outf.write('%f\t%f\t%f\t%f\n' % (even_prior_mean, evenPm, evenGm, evenGGCm))
        print '**'
        print '** Analysis with prior mean',even_prior_mean,'yielded Pm =',evenPm,', Gm =',evenGm
        print '**'
