...
SOURCE_DIR="/Users/mholder/Documents/projects/iterm/iTerm/build/Deployment/"
set -x
files="PhycasGUI.app" 
...
then
	...
	WC_DEV=`hdiutil info | grep "$WC_DIR" | grep "Apple_HFS" | awk '{print $1}'` && hdiutil detach $WC_DEV -quiet -force
	rm -f "$MASTER_DMG"
