...
There are two ways to run Phycas. 
The general way is to invoke python from a terminal.
If you are running the Mac OS you can obtain a \path{Phycas.app} bundle that is built around the open-source terminal iTerm.
\subsubsection{Starting from a terminal}
...
\subsubsection{Starting from the Phycas.app bundle}
If you are using the \path{Phycas.app} bundle (on Mac OS), you can launch Phycas by double clicking on the Phycas icon (this is the \path{.app} bundle, the Mac Finder simply hides the \path{.app} suffix from users).
...
All the instructions for the rest of the manual will be executed the same way regardless of whether Phycas running from a generic terminal or the \path{Phycas.app} bundle.
...
%
) file in the folder (which should contain only the file \path{green.nex}). Name the new file \path{basic.py} and type the following lines into the file:
\begin{verbatim}
from phycas import *
...
To execute the \path{basic.py} script you just created, open a console window, navigate to the directory containing the script and type the following at the command prompt:
%
\begin{verbatim}
python basic.py
\end{verbatim}
