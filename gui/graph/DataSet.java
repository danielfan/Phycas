...
public class DataSet extends Object {
     ...
     */
    private final static int NOLINE    =  0;
    /**
   ...
   /**
     *    The Graphics2D canvas that is driving the whole show.
     * @see graph.Graph2D
     */
    private Graph2D g2d;
 ...
 ******************/
     ...
     * @param g Graphics2D state
     * @param bounds The data window to draw into
     */
    public void draw_data(Graphics2D g, Rectangle bounds) {
        Color c;
        ...
        //if( linestyle != DataSet.NOLINE ) { // add more choices
            if ( linecolor != null) 
                g.setColor(linecolor);
            else                    
                g.setColor(c);
            BasicStroke stroke = new BasicStroke(linewidth,BasicStroke.JOIN_MITER,BasicStroke.CAP_BUTT);
            g.setStroke(stroke);
            draw_lines(g,bounds);
        //}
     ...
     */
    public void setLegend(int x, int y, String text) {
        if(text == null) { 
            legend_text = null;  
            return; 
        }
     ...
     * @param text text to display in the legend
     */
    public void setLegend(double x, double y, String text) {
        if(text == null) { 
            legend_text = null;  
            return; 
        }
 ...
 *********************/
    ...
    protected void draw_lines(Graphics2D g, Rectangle w) {
        int i;
        int j;
        ...
        for(i=stride; i<length; i+=stride) {
            ...
            if(data[i]==0.0 && data[i+1]==0.0) //stop drawing, take out once fix
                break;
 ...
 */
     ...
     * @see graph.Markers
     */
    protected void draw_markers(Graphics2D g, Rectangle w) {
        int x1,y1;
        int i;
        //     Calculate the clipping rectangle
        Rectangle clip = g.getClipBounds();
 ...
 */
    ...
    protected void draw_legend(Graphics2D g, Rectangle w) {
        Color c = g.getColor();
        Markers m = null;
       ...
       // if( linestyle != DataSet.NOLINE ) {
            ...
            g.draw(new Line2D.Double(legend_ix,legend_iy,legend_ix+legend_length,legend_iy));
        ...
        legend_text.draw( g, legend_ix+legend_length+legend_text.charWidth(g,' '),
    ...
    protected void range(int stride) {
        ...
        for(i=stride; i<length; i+=stride ) {
            if(data[i]==0.0 && data[i+1]==0.0) //end of real data
                break;
