...
Do not be confused by the subtle differences in typesetting within this manual in all cases you should use the quotes in Python (not the ``back-tick'' character or any special curved quote that is found in some word-processing programs).
...
Similarly the \code{randomtrees()} invocation returns a \code{TreeCollection} that holds a set of simulate trees (and does not produce visible output).
\end{samepage}
