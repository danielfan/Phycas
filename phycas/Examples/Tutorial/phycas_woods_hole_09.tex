...
Once Phycas is fully installed, you can run \phycas from any terminal by invoking Python and then importing Phycas:
\begin{verbatim}
from phycas import *
\end{verbatim}
...
Our Windows installer puts the files in the standard system-wide location.
The Linux build that we created for the workshop contains a shell script that you can source to set up the shell in the correct way.
After you unpack the archive and set up your environment with the following commands:

{\tt
tar xfvj linux-phycas-1.1.5r1156.tar.bz2\\
cd phycas-1.1.5r1156\\
export PHYCAS\_ROOT=\`{}pwd\`{}\\
source run\_phycas\_env.sh\\
}\\
After setting the PHYCAS\_ROOT and sourcing the script any terminal should be ready to run Phycas.

To run interactively on Linux and use IPython interface (discussed next) you can execute the following command:
\begin{verbatim}
cd phycas-1.1.5r1156
sh phycas.sh
\end{verbatim}
...
Rather than type all of the commands in interactively, this exercise is best done by executing a script.

Open the \localfile{scripts/steppingStone.py} file in a text editor.
The file is heavily commented, so you should be able to read through the file and understand all of the settings.
...
These can be set to \cmdopt{False} or \cmdopt{False} to toggle between the GTR or HKY models and between the steppingstone sampler versus normal MCMC.
At several points in the file you will notice us using Python's if/else branching syntax so we can do slightly different things
...
Be aware that the number of cycles chosen for this lab is too short to provide reliable estimates of the marginal likelihood -- the ncycles was chosen so that the analysis would complete in a reasonable time.
...
\end{verbatim} 
If you are running linux you can call the script by running:
\begin{verbatim}
